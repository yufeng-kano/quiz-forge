...
TITLE_AND_SUMMARY_PROMPT_TEMPLATE = """\
以下是從網頁擷取的正文 Markdown 內容。請用繁體中文完成兩項輸出：

- title：簡潔、可讀的文件標題，以內文主題為準自己下標，不是逐字複製網址或檔名。
- summary：簡短摘要（三到五句話），只用於文件列表與分類顯示，不需要涵蓋所有細節。
