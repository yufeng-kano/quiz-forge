...
- 題庫頁（`docs/decisions/2026-08-17-bank-on-questions-page.md`）：工作區左右出血與頁首底線同寬、toolbar 底下一條分割線（`docs/decisions/2026-08-18-bank-workspace-flush.md` B1／B2）；題庫與已選兩檢視的每題是框線卡片、卡片間有間距（`docs/decisions/2026-08-18-bank-question-cards.md` C1，總覽 `StatCard` 語彙）。左欄兩個檢視——「題庫」（debounce `q`＋語意 `similar_to`＋題型/難度/科目/主題＋真分頁）與「已選」（`exportSelection` 勾選順序，同一套題卡）。沒有「全選目前結果」。選取列只留題數（可切到已選）、前往匯出、清除選取。「新增題目」modal 重用六題型編輯器、可存草稿；每題「複製」「丟棄」用 icon 按鈕（`aria-label` 走 locale）；「管理分類」modal 支援改名/刪除。未向量化提示與「補向量」仍在左欄。題與題是框線卡片（C1），卡片之間有間距。
...
- 品牌 icon 與 tab title：`frontend/public/favicon.svg` 為正式品牌 icon（192×192、`--color-accent` 圓角底＋白色幾何 Q 字樣，取代 Vite 預設 placeholder），另附多尺寸 `favicon.ico`（16/32/48/192）作舊瀏覽器 fallback；browser tab title 由 router `afterEach` 設為「{頁名} · QuizForge」（頁名取 locale `nav.*`／`pages.*` key，無 key 時只用 `app.title`）。
...
- 清單工作區（文件庫、任務中心）必須吃滿頁首以下剩餘視窗高度。篩選列與表格是同一塊工作面，不是另掛一張圓角卡片。列少時表格表面仍拉到底，空白留在表內，不留在卡片下方。高度靠 flex 鏈傳下去：`.app-shell`（grid、`min-height: 100vh`）→ `.app-main`（flex column、撐滿格子）→ `.page.page--workspace`（`flex: 1; min-height: 0`）→ `.workspace` → `DataTable` 的 `fillHeight`。非工作區頁維持自然高度，不套 `page--workspace`。見 `docs/decisions/2026-08-17-list-pages-workspace.md`。題庫是分欄工作區（左清單＋右對話），不是 DataTable 頁；題庫工作區與文件庫同樣左右出血、與頁首底線同寬，toolbar 與清單之間一條分割線（B1／B2）。
- 文件詳情左側頁面大綱與文件庫資料夾欄同一語彙：滿高側欄加分割線，不是獨立圓角卡片。題庫右側對話欄同一語彙。出題表單、匯出表單不套卡片框。**唯一的卡片例外是總覽與用量的總計數字（`StatCard`）**，見 `docs/decisions/2026-08-17-drop-page-titles-keep-stat-cards.md` D24。審題的題目不要做成一疊各自帶完整邊框的漂浮卡；題庫題目是明確例外，為框線卡片（C1）。
- 頁首保持單行緊湊：標題＋計數即可，不放教學句。狀態是文字不是標籤。表格錯誤欄是人話，不是 stack trace。畫面上不常駐操作說明句。空狀態一律一行文字（＋必要時一個動作連結），沒有虛線框。重新整理／重試／移除／新增／改名／展開收合一律 icon-only（`AppButton` 的 `icon` prop），名稱走 `aria-label` 與 `title`；刪除與其他誤點會掉資料的動作保留文字。例外：文件庫資料夾列刪除用 trash icon，按下仍開 ConfirmDialog（L5）。選題助手空狀態只留一行置中淡字「開始對話找出題目」，不要虛線卡片框。輸入框固定列高、不可拉大，沒有「送出」鈕（Enter 送出、Shift+Enter 換行）。送出成功不跳 toast，訊息進時間軸。輸入欄位一體：欄底只放輸入框，輸入框永遠看起來只是輸入欄位；輸入欄位扁平滿寬、無邊框圓角、貼齊欄寬，與對話區只留欄底一條 hairline 分隔（H2，`docs/decisions/2026-08-18-bank-agent-composer-flat-fullwidth.md`）；回合進行中的進度與失敗重試顯示在對話區底部（一行淡色文字、不是訊息、無卡片框，結束即消失），`turnRequestError` 例外維持在輸入框上方（H1，`docs/decisions/2026-08-18-bank-agent-progress-in-conversation.md`）。
